...
const userData = [
  ...
  {
    name: "withdrawal_amount_currency_code",
    label: "Withdrawal currency:",
    placeholder: "Enter Withdrawal currency code",
    disabled: true,
  },
];

const WithdrawalStaff = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    setUserObject({
      ...
      currency_code: data?.data?.user?.currency_code || "N/A",
      total_withdrawal_amount:
        data?.data?.user?.total_withdrawal_amount || "N/A",
      withdrawal_amount_currency_code:
        data?.data?.user?.withdrawal_amount_currency_code || "N/A",
    });
  }, [JSON.stringify(data)]);
...
export default WithdrawalStaff;
